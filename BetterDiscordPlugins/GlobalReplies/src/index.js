...
const MessageContextMenu = get(m => m?.default?.displayName === "MessageContextMenu");
			...
			children={
				channels.SELECTABLE.filter(e => GuildPermissions.can(Permissions.SEND_MESSAGES, e.channel)).map(e => <Menu.MenuItem
					...
					action={() => {
						...
						ComponentDispatch.dispatchToLastSubscribed("INSERT_TEXT", {content: Utilities.formatString(this.getTemplate(), {messageLink: `https://discord.com/channels/${server.id}/${channel.id}/${props.message.id}`, author: `<@${props.message.author.id}>`, authorTag: props.message.author.tag, channel: `<#${channel.id}>`})})
					}}
					label={[
