	...
	changelog: [
		{
			...
			type: 'Group DM fix',
			items: [
				'Fixed Group DMs not showing notifications/crashing while there is no group name. They show up properly now (shows usernames of people in group)',
			],
		},
	],
	defaultConfig: [
				...
				onMessage({ message }) {
					...
					if (channel.type === ChannelTypes['GROUP_DM']) {
						...
						if (!channel.name || channel.name === ' ' || channel.name === '') {
							authorString = `${author.tag} (${channel.rawRecipients
								.map((e) => e.username)
								.join(', ')})`;
						}
					}
